...
// put them in a select drop-down
                ...
                while ($Store = $stmt->fetch(PDO::FETCH_ASSOC)) {
                    ...
                    if ($alert->store == $Store) {
                        echo "<option value='$alert->store' selected>";
                    } else {
...
// put them in a select drop-down
                echo "<select class='form-control' name='appName'>";

                echo "<option>Please select...</option>";
                while ($appName = $stmt->fetch(PDO::FETCH_ASSOC)) {
                    extract($appName);

                    // current category of the product must be selected
                    if ($alert->app_name == $appName) {
                        echo "<option value='$appName' selected>";
                    } else {
                        echo "<option value='$appName'>";
                    }
...
// put them in a select drop-down
        ...
        <tr>
            ...
            <td>
                <!-- step="0.01" was used so that it can accept number with two decimal places -->
                <input type='text' name='appID' value="<?php echo htmlspecialchars($alert->app_id, ENT_QUOTES, 'UTF-8'); ?>" class='form-control' required />
            </td>
        </tr>
